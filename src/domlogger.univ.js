...
function DOMLogger(optionsParam = {}) {
	...
	function generateLogFunc(
		msgType,
		classNameComponent,
		classNameMessage,
		modifierFunc = undefined
	) {
		return function(msgParam, isHTML = false) {
			...
			logger._.data.blinkedTimeoutId = setTimeout(function() {
				if (logger._.data.blinkedTimeoutId) {
					logger._.component.className = logger._.component.className.replace(
						classNameComponent,
						""
					);
				}
	 ...
	 *
	 */
	logger.registerMessageType = function(
		methodName,
		classNameComponent = "",
		classNameMessage = "",
		modifierFunc = undefined,
		msgTypeAbbr = undefined
	) {
		logger[methodName] = generateLogFunc(
			msgTypeAbbr ? msgTypeAbbr : methodName.toUpperCase(),
			classNameComponent,
			classNameMessage,
			modifierFunc
		);
